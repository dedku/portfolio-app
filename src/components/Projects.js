...
const Projects = () => {
    ...
    <Container maxW="container.xl" id="projects">
        ...
        { data && <Motion delay={0.2}><SimpleGrid
        ...
        >
            {data.map(i => { return <Box role='group'  key={i.projectId}>
                <Box h={{ base: 'calc(50vh - 3rem)', md: 'calc(50vh -  3rem)' }} maxH='350px' minH='280px' rounded='xl' position='relative' onMouseOver={()=> setHovered(i.projectId)} onMouseOut={()=> setHovered(null)}>
                    <Box bgImage={i.imageUrl} bgSize='cover' rounded='xl' position="absolute" top='0' right='0' left='0' bottom='0'  zIndex='-2' opacity='0.85' transition='0.5s' _groupHover={{ opacity: '0.1', transition: '0.3s'}} />
                    <Flex>
                        ...
                        <Box  m={4} p={2} rounded='xl' bg="blackAlpha.900" h='100%'>
                            <Flex gap={2}>
                                ...
                                {i.icons.React && <FaReact  color='#007ACC' fontSize={20} />}
                    ...
                    <VStack gap={{ 'sm': 1, 'md': 4}}>
                        <Motion initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} >
                        ...
                        <Motion delay={0.1} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} w='100%'>
                            <Badge colorScheme={i.progresColor} rounded='xl' p='6px' mb={4}>{i.projectProgres}</Badge>
                            <Divider w={64}/>
                        </Motion>
                        <Motion delay={0.2}  initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} mx='auto'>
                        <Text mx={14} > {i.projectDesc}  </Text>
                        </Motion>
                        <Motion delay={0.3} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} >
